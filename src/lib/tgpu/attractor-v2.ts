...
const initCompute = t3.toTSL(() => {
  ...
  const basePosition = std.mul(
    d.vec3f(randf.sample() - 0.5, randf.sample() - 0.5, randf.sample() - 0.5),
    d.vec3f(5, 0.2, 5)
  )
  ...
  const baseVelocity = std.mul(
    d.vec3f(sinPhiRadius * std.sin(theta), std.cos(phi), sinPhiRadius * std.cos(theta)),
    0.05
  )
...
const reset = () => {
  ...
  return renderer.compute(initCompute.compute(count))
}
...
const update = t3.toTSL(() => {
  ...
  for (let i = d.u32(0); i < attractorsLength.$; i++) {
    ...
    // spinning
    const spinningForce = std.mul(attractorRotationAxis, gravityStrength * spinningStrength.$)
  ...
  velocity = std.mul(velocity, 1 - velocityDamping.$)
...
updateCompute.setName('Update Particles')

// nodes

material.positionNode = positionBuffer.node.toAttribute()

// t3.toTSL's declared signature always returns a generic Node, regardless of the
// callback's actual return type, so the concrete node type is restored via assertion.
...
material.scaleNode = mul(t3.toTSL(getParticleMassMultiplier) as unknown as THREE.Node<'float'>, scale)
